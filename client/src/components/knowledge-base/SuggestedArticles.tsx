...
export default function SuggestedArticles({
  ...
  return (
    <section className={`mt-6 rounded-2xl border p-4 ${isDark ? "border-gray-800 bg-gray-950" : "border-slate-200 bg-slate-50"}`}>
      ...
      {results.length > 0 && <div className="mt-4 space-y-2">{results.map((result, index) => <div key={`${result.article_id}-${result.chunk_index ?? index}`} className={`rounded-xl border p-3 ${isDark ? "border-gray-800 bg-gray-900" : "border-slate-200 bg-white"}`}><p className={`text-sm font-semibold ${isDark ? "text-white" : "text-slate-900"}`}>{result.article_title || "Knowledge article"}</p><p className={`mt-1 text-xs ${isDark ? "text-gray-400" : "text-slate-500"}`}>{result.category || "Support knowledge"}{result.heading_path ? ` · ${result.heading_path}` : ""}</p>{result.content && <p className={`mt-2 line-clamp-2 whitespace-pre-wrap text-xs leading-5 ${isDark ? "text-gray-300" : "text-slate-600"}`}>{result.content}</p>}</div>)}</div>}
    </section>
  );
}
